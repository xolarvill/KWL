...
def _run_estimation_with_tracking(tracker, sample_size, use_bootstrap, n_bootstrap, bootstrap_jobs, stderr_method, em_parallel_jobs, em_parallel_backend, lbfgsb_gtol=None, lbfgsb_ftol=None, lbfgsb_maxiter=None, strategy="normal"):
    ...
    def load_and_prepare_data():
        ...
        logger.info("\n数据准备完成。")
        return data_loader, distance_matrix, adjacency_matrix, df_individual, state_space, transition_matrices, df_region
    
    data_result = resume_estimation_phase(
        tracker, "data_preparation", load_and_prepare_data
    )
    data_loader, distance_matrix, adjacency_matrix, df_individual, state_space, transition_matrices, df_region = data_result
    
    # --- 3. 模型估计 ---
    def perform_model_estimation():
        logger.info("\n开始模型估计...")
        # 从ModelConfig获取初始参数
        ...
        logger.info("\n估计完成。")
        return results, support_gen
    
    estimation_result, support_gen = resume_estimation_phase(
        tracker, "model_estimation", perform_model_estimation
    )
    
    # 提取估计结果
    estimated_params = estimation_result["structural_params"]
    final_log_likelihood = estimation_result["final_log_likelihood"]
    ...
    std_errors, t_stats, p_values = resume_estimation_phase(
        tracker, "standard_error_computation", compute_standard_errors
    )
    ...
    def output_results():
        ...
        return info_criteria, model_fit_metrics
    
    info_criteria, model_fit_metrics = resume_estimation_phase(
        tracker, "result_output", output_results
    )
