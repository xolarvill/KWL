...
我确实有一个专门用于存放计算细节的section，放在介绍完异质性、ML插件和EM-NFXP方法之后。在这一章，我想要专门来讲一下为了优化计算我设计了哪些专门的地方，例如使用向量化、joblib多线程、机器学习中的LightGBM等内容。有这个必要吗？请你帮我评估
...
\item 使用joblib多线程并行计算减轻计算压力
\item 向量化vectorization计算
\item 同时本文还使用了机器学习方法，对于条件分布和条件均值，使用LightGBM进行优化，并且使用K折交叉拟合来防止过拟合。
\item 在 NFXP 的前向模拟/期望中调用 ML 预测器
...
虽然L-BFGS优化器在一定程度上缓解了这一问题，但仍需根据经济学理论进行初始值赋予能促进优化方法的应用，例如距离增加可能降低迁移概率，因此\lstinline{gamma_distance}的预期应该为负，将其初始值设为负符合理论预期，有助于引导优化方向。hhhhh
...
标准回归下得到的估计参数如下表格\ref{tab:结构参数的估计结果}所示。（附带标准误和显著性星号标记）
\begin{table}[!ht]
\centering
\caption{结构参数结果}
...
我们报告 Hit Rate、交叉熵和 Brier Score；并对关键分组的选择份额做误差分解。
...
抽样型映射：这就是把有限混合类型抽到 ABM 里，每类人带着不同的参数，保持和你微观估计一致。
