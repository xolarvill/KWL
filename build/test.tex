...
\section{地区平均工资的分解} % (fold)
\label{sec:地区平均工资的分解}
为了在模型中精确地刻画驱动劳动力流动的地区间经济差异，我们需要构建一个能反映各地纯粹生产力水平的指标——即随时间动态变化的地区基础工资 $\mu_{jt}$。直接使用官方发布的地区平均工资数据是不恰当的，因为它混淆了地区真实的生产力效应和在该地区工作的人口构成效应 (composition effects)。例如，一个地区的平均工资高，可能是因为该地生产力确实高，也可能仅仅是因为该地恰好聚集了更多高学历、高技能的个体。

为解决这一关键的识别问题，我们采用工资分解 (wage decomposition) 的方法，通过一个两步流程来估计 $\mu_{jt}$：

第一步：通过面板回归分解工资

我们利用CFPS微观面板数据，进行了一项工资分解回归。该回归旨在将观测到的个体对数工资 $\ln(w_{itj}^{\text{obs}}$) 分解为三个主要部分：可观测的个体时变特征、不随时间改变的个体异质性，以及我们最关心的“地区-年份”联合固定效应。具体回归模型设定如下：
\begin{equation}
\ln(w_{itj}^{\text{obs}}) = \beta' X_{it} + \alpha_i + \delta_{jt} + \varepsilon_{itj}
\end{equation}
其中，$X_{it}$ 包含了一系列标准的Mincer式时变控制变量，如年龄及其平方项。$\alpha_i$ 是个体固定效应，用于吸收所有不随时间改变的个体不可观测特征（如个人能力、家庭背景等）。
核心在于 $\delta_{jt}$，这是一组“地区 × 年份”的完全交互固定效应。我们为样本中的每个地区 j 在每个年份 t 都设定了一个独立的虚拟变量。这是一种高度灵活的非参数方法，能够吸收掉在该时期该地区发生的所有宏观层面冲击对工资的影响。

第二步：提取并阐释 $\mu_{jt}$

在上述回归完成后，我们提取出的“地区-年份”交互固定效应的估计系数 $\hat{\delta}_{jt}$，即被用作我们主结构模型中地区基础工资的代理变量，即 $\mu_{jt} \equiv \hat{\delta}_{jt}$。
该系数的经济学含义是：在控制了个体所有可观测时变特征和所有不可观测的持久性特征后，仅仅因为在时期 t 身处地区 j 所能带来的平均对数工资溢价。它干净地度量了地区劳动力市场的“基础水位”或“地理位置租金”(locational rent)，反映了由地区特有的产业结构、技术水平和地方政策等共同决定的纯粹经济吸引力。
通过这种方式，我们得到了一个捕捉地区经济动态演变的面板数据集 $\{\mu_{jt}\}$。它将作为一项可观测的地区特征，输入到我们的动态迁移决策模型中。这一严谨的预处理步骤，为后续模型准确识别迁移成本、户籍壁垒等摩擦因素的大小奠定了坚实的基础。
...
1. 工资生成插件: 我们的工资预测模块由两部分构成。第一部分是前述通过工资分解回归得到的地区基础工资 $\mu_{jt}$，它捕捉了所有的地区宏观效应。第二部分则是纯粹由个体因素决定的工资部分，包括可观测特征、不可观测的持久能力 $\eta_i$ 和地点匹配效应 $\nu_{ij}$ 等。
为了灵活地捕捉这些个体因素之间复杂的非线性与交互关系，我们训练一个梯度提升树模型 $\hat{g}(\cdot)$，用个体的完整状态向量 $x_{it}$（不含当前地理位置）来预测剔除了地区效应后的残差工资 $\ln(w_{itj}^{\text{obs}}) - \mu_{jt}$。

最终，在我们的结构模型中，个体对任一潜在地区 j 的（对数）工资预期由这两部分相加构成：
\begin{equation}
\widehat{\ln(w_{itj})} = \mu_{jt} + \hat{g}(x_{it})
\end{equation}
这一组合方法既利用了面板固定效应在分离宏观因素上的强大能力，又利用了机器学习在拟合微观复杂交互上的预测优势，从而构建了一个高度稳健的工资预测模块。
