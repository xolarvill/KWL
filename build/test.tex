...
\centering
% --- 此处放置您并列的两个PDP图 ---
\includegraphics[width=\textwidth]{../results/ml_comparison/age_partial_dependence_plot.png} % 替换为您的图片路径
...
% --- 此处放置您的图表 ---
% 建议使用一个散点图或热图
% 例如：一个3D散点图，X, Y, Z轴分别是 gamma_0_type_0, gamma_0_type_1, gamma_0_type_2 的值，
% 点的颜色或大小代表 'balance_score' 或 'bic' 的值。
% 或者一个二维热图，固定一个gamma值，另外两个作为X,Y轴。
%\includegraphics[width=0.8\textwidth]{images/placeholder_stage1_search.png} % 替换为您的图片路径
...
经过NFXP算法估计得到的参数值如下表格\ref{结构参数估计结果}所示。（附带标准误和显著性星号标记）
