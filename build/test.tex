...
影响劳动力迁移的因素如表格\ref{tab:影响劳动力迁移的因素}归纳。

\begin{table}[!ht]
\centering
\begin{tabular}{@{}lll@{}}
\toprule
\multicolumn{1}{c}{\textbf{影响劳动力迁移的因素}} &  & \multicolumn{1}{c}{\textbf{主要讨论文献}} \\ \midrule
收入差异 &  &  \\
房价 &  &  \\
户籍 &  &  \\
公共服务 &  &  \\
...
考虑一个理性的决策者在$T$期生命周期内选择最优居住地序列。令$\mathcal{C} = \{1,2,\dots,m\}$表示居住地选择集，$j_t \in \mathcal{C}$表示第$t$期的居住地选择，那么完整的选择序列可表示为$\mathcal{J}=\{j_1, j_2 ,\dots,j_T\}$。典型的居住地选择模式如表格\ref{tab:居住地选择序列可能的形式}所示：
...
\end{equation}
其中$u_t(j, x_t)$为可观测效用项，$\zeta_{jt}$为在任意时期和地点都独立同分布的随机扰动项，服从标准Gumbel分布$F(\zeta) = \exp(-\exp(-\zeta))$，且与状态变量$x_t$无关。
...
图片\ref{fig:migration_flow_resized} 展示了一个动态规划决策过程，其中在上个离散周期$t-1$结束时或者下个周期$t$即将开始时，决策者根据在随机收益冲击$\zeta$和状态转移概率$p(x_{t+1}|x_t,j_t)$双重不确定性的影响下通过Bellman方程做出决策，进入下一个周期$t$后获得效用。

\begin{figure}[!ht]
\centering
\includegraphics[width=0.9\textwidth]{images/dynamicsequence.drawio.pdf}
...
\begin{equation}
  u_t^h(x_t,j_t)=\alpha_0 \cdot w(j_t,\omega_t)+\sum\limits_{s} \alpha_{s} \cdot y_{s}(j_t)+\alpha_h \cdot I(j_t=h)+\xi(j_t,\omega)-\kappa
\end{equation}
其中$w(j_t,\omega_t)$为经济收益，衡量个体在当前位置通过劳动获得的经济收益，$\alpha_0$表示工资收入对效用的权重（边际效用）；
$y_{s}(j_t)$为该居住地提供的非货币性舒适度（amenity）价值，如气候、公共服务、文化等，反映地理位置的非经济吸引力，$y_s$是第$s$种舒适度的量化指标（如教育质量指数），其边际效用由$\alpha_s$表示；
$\alpha_h \cdot I(j_t=h)$是对家乡的偏好溢价，$\alpha_h$衡量个体对出生地的情感依恋强度，其中$I(\cdot)$为指示性函数，当其中判断为真时返回1，否则为0，这意味着若当前所在地是家乡则效用增加$\alpha_h$，否则为0；
$\xi(j_t,\omega)$为与地理位置匹配的永久性随机效用成分，
反映个体对某地点的长期偏好匹配（如文化契合度），仅当个体访问该地点时才能获知其具体值（不完全信息），
类似工资中的匹配项 $\nu$，但仅能通过迁移选择推断，而 $\nu$可通过工资和迁移选择共同推断；
$-\kappa$为从其他区域前往地区$j$的迁移成本，负号表示迁移成本会降低效用，包括直接费用（如搬家费）和间接成本（如适应新环境的心理成本）。



\subsection{收入}

决策者的经济收益$w_{ij}$由多种因素组成。假设个体决策者$i \in \mathcal{N}$在地区$j \in \mathcal{C}$的收入设定为以下函数形式：
\begin{equation}
  w_{ij}(a)=\mu_j + \nu_{ij} + G(X_i,a,t) + \eta_i + \varepsilon_{ij}(a)
\end{equation}
...
不同 $\tau$ 对应不同 $\gamma_0$，需通过数据估计类型分布。
...
参数的经济含义如表格\ref{tab:迁移成本参数释义}所示：
\begin{table}[!ht]
  \centering
  \begin{tabular}{@{}lll@{}}
    \toprule
    \multicolumn{1}{c}{\textbf{参数符号}} & \multicolumn{1}{c}{\textbf{参数含义}} & \multicolumn{1}{c}{\textbf{参数意义}} \\ \midrule
    \multicolumn{1}{c}{$\gamma_{0\tau}$} & 类型异质效应 & \begin{tabular}[c]{@{}l@{}}捕捉不同类型的异质性，可以解释为何有些人从不迁移，识\\ 别“定居型”人群可针对性地设计激励措施（如搬迁补贴）。\end{tabular} \\ 
    ...
    \multicolumn{1}{c}{$\gamma_4$} & 年龄障碍效应 & \begin{tabular}[c]{@{}l@{}}针对不同年龄群体制定差异化迁移政策（如青年人才引进\\ 计划）。\end{tabular} \\ 
    \multicolumn{1}{c}{$\gamma_5$} & 规模成本效应 & \begin{tabular}[c]{@{}l@{}}大地区吸引力不仅体现在经济机会（如工资 $\mu_j$），也\\ 直接降低迁移成本。\end{tabular} \\ \bottomrule
  \end{tabular}
  \caption{迁移成本参数释义}
  \label{tab:迁移成本参数释义}
\end{table}
...
此概率描述的是在选择了特定位置$j_{t+1}$的情况下，从一个状态 $x_t$ 移动到新状态 $x_{t+1}$的可能性。这是一个条件概率，它关注状态如何根据位置决策而变化的机制。它既包括确定性变化（如年龄增长），也包括随机变化（如位置匹配效应的随机抽取）。
...
不同地区的选择就会对收入产生影响，
劳动力会因为更好的劳动力市场、更好的地区匹配效应（包括工资与偏好）以及随机效用冲击而选择迁移。

至此，模型刻画出了在有限生命周期内，每期个体可以在不同城市$\mathcal{C}$中自由选择最优住址$j_t$，这些不同时期的选择构建出了居住地选择序列$\mathcal{J}$，这意味着迁移可以是多次的，可撤回的。图片\ref{fig:migration_flow_resized2}展示。

\begin{figure}[!ht]
\centering
\includegraphics[width=0.9\textwidth]{images/dynamicsequence2.drawio.pdf}
\caption{动态离散选择模型下的劳动力迁移决策流程图2}
...
\subsection{location match effect}
由于该模型存在大量的（）
所以出于计算上的简便
我们使用离散估计替代一些变量
...
蒋琪2018劳动经济研究：
\textit{本研究使用CFPS2010年和2014年两期数据构成面板进行影响评估。CFPS采用的是内隐分层、多阶段、多层次、与人口规模成比例的概率抽样方式，样本覆盖除中国港澳台、新疆、西藏、青海、内蒙古、宁夏和海南之外的其他省/市/自治区。这些地区的人口占全国的95\%左右，因此，CFPS的数据是一个具有全国代表性的高质量数据库 (Xie \＆ Lu，2015)}
