...
$$
其中$\theta$为理论模型中的结构参数向量；$\pi_\tau$为有限混合模型中各类型 $\tau$ 的人口占比，$\sum_{\tau=1}^K \pi_\tau = 1$；支撑点位置与概率是对应未观测异质性组合 $\omega = \{\eta_i, \{\nu_{ij}\}, \{\xi_{ij}\}, \sigma_{\varepsilon,i}\}$ 的离散化近似。

在E步，我们给定当前对参数向量$\Theta^{(k)}$的一组猜测值\footnote{此处$ \theta^{(t)} $的上标表示在第 $ t $ 次迭代开始时使用的参数，用于E步计算。$ \theta^{(t+1)} $ 是在第 $ t $ 次迭代的M步后得到的参数，用于下一轮E步。}，需要计算每个个体$i$属于特定潜在类别$\tau$，并拥有特定未观测属性组合$\omega_i$的后验概率。根据贝叶斯法则，该后验概率为：
\begin{equation}
...
\end{equation}
并在常规极大似然估计的正则条件下保证一致性与渐近正态性。计算上，Louis 方法仅依赖于E步输出的中间量即可完成，无需多次调用NFXP内层循环，可以显著减少计算时间。
