...
\begin{equation}
    \begin{split}
        \text{HukouPenalty}(j, h_i, K_t) ={}& I(j \neq h_i) \cdot \bigl[ \sum\limits_{c \in C} \rho_{\text{base}, c} \cdot \text{Tier}_{c(j)}+ \rho_{\text{edu}} \cdot \text{Edu}(j) \\
...
我们提取出的这一系列 $\hat{\delta}_{jt}$ 系数，便构成了我们主模型中使用的地区基础工资水平 $\mu_{jt}$。通过这种方式，我们得到了一个干净的、能够反映地区间真实经济动态差异的指标。
